...
(function() {
	...
	Probability.getInvocationProbabilityFromSkill = function(unit, skill) {
		switch(skill.getSkillType()) {
			case SkillType.REACTION:
				//If it is a command skill, it will not be activated if it has not been activated.
				...
				if(!EC_SkillCheck.isSkillCheckEnable(unit, skill, true, true)) {
					return false;
				}
				break;
		}
	...
	//=== The number of weapon uses does not decrease ==========================
		...
		switch(skilltype) {
			case SkillType.FUSION:
				...
				for (var i = 0; i < count; i++) {
					...
					//Add those that meet the activation conditions to the list
					if(EC_SkillCheck.isSkillCheckEnable(unit, skill)) {
